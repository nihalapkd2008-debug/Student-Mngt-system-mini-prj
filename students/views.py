...
def add_student(request):
    if request.method == 'POST':
        ...
        if Student.objects.filter(email=email).exists(): errors.append("A student with this email already exists.")
...
def search_students(request):
    ...
    if query:
        students = Student.objects.filter(
            ...
            Q(email__icontains=query) | Q(class_name__icontains=query) |
    ...
    data = [{
        ...
        'address': s.address, 'admission_date': str(s.admission_date)
    } for s in students]
...
def student_profile(request, student_id):
    ...
    # Attendance - CORRECT: Count first, then slice for display
    present_count = student.attendances.filter(status='Present').count()
    absent_count = student.attendances.filter(status='Absent').count()
    leave_count = student.attendances.filter(status='Leave').count()
    
    # For display - slice after getting all
    attendances = student.attendances.all()[:30]
    
    # Fee calculations
    total_fees = fees.aggregate(Sum('amount'))['amount__sum'] or 0
    paid_fees = fees.filter(status='Paid').aggregate(Sum('amount'))['amount__sum'] or 0
...
def fee_list(request):
    ...
    if student_filter:
        fees = fees.filter(student__name__icontains=student_filter)
    ...
    pending_amount = fees.filter(status='Pending').aggregate(Sum('amount'))['amount__sum'] or 0
    overdue_amount = fees.filter(status='Overdue').aggregate(Sum('amount'))['amount__sum'] or 0
...
def update_fee(request, fee_id):
    ...
    if request.method == 'POST':
        ...
        fee.student = get_object_or_404(Student, id=student_id)
        fee.amount = amount
    ...
    students = Student.objects.all()
    return render(request, 'add_fee.html', {'fee': fee, 'students': students})
...
def delete_fee(request, fee_id):
    ...
    if request.method == 'POST':
        fee.delete()
        messages.success(request, 'Fee record deleted! 🗑️')
        return redirect('fee_list')
    return redirect('fee_list')
...
def attendance_list(request):
    ...
    student_filter = request.GET.get('student', '')
    if student_filter:
        attendances = attendances.filter(student__name__icontains=student_filter)
